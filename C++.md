...
在C++中，`std::weak_ptr` 是一种智能指针，用于解决循环引用的问题。`std::weak_ptr` 允许你观测（watch）由 `std::shared_ptr` 管理的对象，而不会增加对象的引用计数。当需要使用被观测对象时，可以通过调用 `lock` 成员函数来获取一个有效的 `std::shared_ptr`，如果对象存在的话。

`std::weak_ptr` 的 `lock` 成员函数的底层原理如下：

1. `std::weak_ptr` 内部包含了一个底层的控制块（control block），其中存储了被 `std::shared_ptr` 管理对象的引用计数信息。

2. 当调用 `lock` 函数时，它会检查底层的控制块是否还有效。如果控制块已经被析构或者 `std::shared_ptr` 释放了管理对象的所有权，那么 `lock` 函数将返回一个空 `std::shared_ptr`。

3. 如果控制块仍然有效，`lock` 函数会创建一个新的 `std::shared_ptr` 对象，该对象与 `std::weak_ptr` 共享相同的控制块。这将增加被观测对象的引用计数，确保对象在 `std::shared_ptr` 存在时不会被提前销毁。

总而言之，`lock` 函数通过检查底层控制块是否有效来决定返回一个有效的 `std::shared_ptr` 或空的 `std::shared_ptr`。如果底层控制块有效，它会创建一个新的 `std::shared_ptr` 并与 `std::weak_ptr` 共享相同的控制块，从而确保对象在
...
![img](C++.assets/v2-ef842c65b84bf82f773ab4b655387a3a_720w.webp)

PRT思想：将光照切割成Lighting, Light Transport两部分。

![img](C++.assets/v2-ac89de64010d5e44c6965bde98867b83_720w.webp)

- 我们把rendering equation分为两部分,lighting 和 light transport.
- 对于渲染方程，里面的光照项、可见项、BRDF项均可表面为球面谐波函数函数

![img](C++.assets/v2-2951b777b686f4c78118d7e842763074_720w.webp)

####  diffuse物体

- BRDF项为常量，光照项表示为球面谐波函数并将光照项在球面谐波函数下的系数提取出来

![img](C++.assets/v2-063db9e7d2ec8701b04a7cec91f4bfbb_720w.webp)

- 可见项与基函数的积分相当于对可见项进行投影

- 可以理解成对每个光照基函数做一遍光照

![img](C++.assets/v2-1499b4c16aee149846108b79d6cd63b7_720w.webp)
...
![img](C++.assets/v2-c11f46219d43de1a0d6fcf6e87ef0994_720w.webp)
...
基本绘制单元：Plane 16 X 16 (8m x 8m)，每个网格精度是0.5m，一个sector一个 8 x 8个绘制单元 

1. 分为六级LOD，LOD0 Node 一个Node大小为一个Sector，160 X160，LOD5用5X5表示 。
2. 首先建立四叉树，四叉树判断的条件是根据这个Node的中心点位置以及平均高度和摄像机位置的值大小进行判断，当满足划分条件的时候，将四个子节点记录到list里面，LOD5判断完以后，将list作为输入，判断lod4的信息，依次完成，直至LOD0，至此。就建立了一个四叉树数据，存放了场景不同位置的Node信息。
3. 在确定完LOD等级以后，然后在Node上进行8*8的划分，划分成更加精细的patch单元，LOD0里面的一个patch就是一个Plane，其他的就是等比方法Plane得到的图元结构。根据patch生成一个Bound包围盒，完成遮挡剔除和视锥剔除，如果都通过了加入到需要绘制的patch信息列表中，最终经过验证，在不做裁剪的情况下，可以用5000个patch绘制出所有地形，按照原始方法的话原本需要160w个patch，在做完视锥剔除以后，大概减少到2000个，再遮挡剔除后大概再1200左右。
...
![image-20240105195013711](C++.assets/image-20240105195013711.png)
...
![image-20240105195054625](C++.assets/image-20240105195054625.png)
...
![image-20240105200738394](C++.assets/image-20240105200738394.png)
